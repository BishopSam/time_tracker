...
class EntryListItem extends StatelessWidget {
  ...
  Widget _buildContents(BuildContext context) {
    ...
    final durationFormatted = entry.durationInHours >= 1
        ? Format.hours(entry.durationInHours.toDouble())
        : entry.durationInMinutes;

    final pay = job!.ratePerHour * entry.durationInHours;
    final duration = entry.durationInMinutesInt;
    final payforMins = job!.ratePerHour * (duration / 60);
    final payFormatted = Format.currency(pay.toDouble());
    final payformattedMins = Format.currency(payforMins);
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        Row(children: <Widget>[
          ...
          Expanded(child: Container()),
          Text(durationFormatted.toString(), style: TextStyle(fontSize: 16.0)),
        ]),
